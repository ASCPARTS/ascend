...
switch ($strProcess) {
    case 'Filter':
        $jsnPhpScriptResponse = array('strTitle'=>'BackOrders por Artículos (Compras)','arrFilters'=>array());

        $strFilter = '<div class="col-xs-1-1 col-sm-1-2 col-md-1-4 col-md-1-4 col-lg-1-5">';
        $strFilter .= '<div class="divInputText barCodeGray">';
        $strFilter .= '<input type="text" id="strFolio" maxlength="7">';
        $strFilter .= '<label>Folio Pedido</label>';
        $strFilter .= '</div>';
        $strFilter .= '</div>';
        array_push($jsnPhpScriptResponse['arrFilters'],array('name'=>'strFolio','label'=>'Folio','html'=>$strFilter,'type'=>'numeric','negative'=>'','decimalPlaces'=>'','required'=>''));
