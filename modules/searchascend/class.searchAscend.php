...
class clsSearchAscend extends clsAscend
{
    ...
    {
        $htmlLateralBar = "";
        $strSelectPivot = "SELECT P.intId, P.strSku, P.strPartNumber, P.strDescription, P.decPrice, P.intBrand, B.strName AS strBrand, P.intGroup, C.strName AS strCondition, I.intSold, IFNULL(PR.strRule, '') AS strPromotionRule, IFNULL(PR.strStatus, 'B') AS strPromotionStatus, IFNULL(PI.strUrl, 'product/notfound.jpg') AS strImage, IFNULL( (SELECT SUM(intStock) FROM tblWarehouseStock WHERE intProduct = P.intId AND strStatus = 'A'), 0) AS intStock";
        ...
        $htmlLateralBar .= '<input type="checkbox" id="stocksFilter" ' . ( $intStock == 1 ? 'checked' : '' ) .' >';
        ...
        $htmlLateralBar .= '<select id="priceRangeFilter"> ';
        ...
        if( count($objBrands) > 0 )
        {
            ...
            foreach( $objBrands as $brand )
            {
                ...
                $htmlLateralBar .= '<div class="divInputMiniCheck divBrandBlue">';
                $htmlLateralBar .= '<input type="checkbox" id="' . 'chbStrBrand' . $brand["intBrand"] . '" ' . ( $intIsCheckedBrand == 1 ? 'checked' : '') . '>';
        ...
        if( count($objGroups) > 0 )
        {
            ...
            foreach( $objGroups as $group )
            {
                ...
                $htmlLateralBar .= '<div class="divInputMiniCheck divBrandBlue">';
                $htmlLateralBar .= '<input type="checkbox" id="' . 'chbStrGroup' . $group["intGroup"] . '" ' . ( $intIsCheckedGroup == 1 ? 'checked' : '') . '>';
